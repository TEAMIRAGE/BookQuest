...
module.exports.homePage_get = async (req,res)=>{
     ...
     try{
        ...
        const popularBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=current-popular-books`);
        const popularBookDetails = popularBookResponse.data;

        const popularBookSet = await Promise.all(
            popularBookDetails.map(async (info)=>{
          const [signedURL] = await bucket.file(info.image_bucket_name).getSignedUrl({
          action:'read',
          expires :Date.now() + 60 * 60 * 1000,
          });
        
          return {signedURL, name : info.name, author: info.author, id: info._id};
        ...
        const latestBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=latest-books`);
        ...
        const firstYearBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=firstYearBooks`);
        ...
        const secondYearBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=secondYearBooks`);
        ...
        const thirdYearBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=thirdYearBooks`);
        ...
        const fourthYearBookResponse = await axios.get(`${ip}${port}/api/all_books_details?id=fourthYearBooks`);
       ...
       res.render('home.ejs', { popularBookSet, latestBookSet, firstYearBookSet, secondYearBookSet, thirdYearBookSet, fourthYearBookSet });
...
const options = {
  ...
  const [signedUrl] = await bucket.file(details.image_bucket_name).getSignedUrl(options);
...
module.exports.specific_book_buy_page_get = async (req,res)=>{
  try{
    ...
    const options = {
      ...
      const [signedUrl] = await bucket.file(details.image_bucket_name).getSignedUrl(options);
     details.profileUrl = signedUrl;
     const alreadyRequestedResponse = await request_details_books.findOne({student_auth_id:userID, book_details_id:details._id});
...
module.exports.student_profile_get = async (req, res) => {
  try {
    ...
    const [signedUrl] = await bucket.file(studentInfo.image_bucket_name).getSignedUrl(options);
    studentInfo.profileUrl = signedUrl;

    const collectionInfo = await student_book_storage.find({ student_auth_id: studentInfo._id });

    if (collectionInfo.length === 1) {
      studentInfo.collectionStatus = 1;
      const manualIdInfo = await all_books.findOne({ admin_id_manual: collectionInfo[0].admin_id_manual_list });

      if (manualIdInfo) {
        const bookInfo = await book_details.findById(manualIdInfo.book_details_id);

        if (!bookInfo) {
          throw new Error('Book details not found');
        }

        const [signedUrl] = await bucket.file(bookInfo.image_bucket_name).getSignedUrl(options);
      ...
      const manualIdInfo = collectionInfo.map((studentBook) => studentBook.admin_id_manual_list);
      const books = await all_books.find({ admin_id_manual: { $in: manualIdInfo } });
      const bookDetailIds = books.map((book) => book.book_details_id);
      const bookDetails = await book_details.find({ _id: { $in: bookDetailIds } });
      ...
      const details_all = await Promise.all(
        bookDetails.map(async (info) => {
          ...
          return { signedURL, name: info.name, author: info.author, id: info._id, manualID: adminId };
        })
      );
    ...
    console.error(err);
    res.status(500).send({ message: err.message || 'Some error occurred while performing this operation' });
  }
};
...
const response = await axios.get(`${ip}${port}/student-notifications-common?studentId=${userID}`);
const details = response.data;
...
module.exports.book_request_list_get = async (req, res) => {
  try {
    ...
    if (details.length > 0) {
      ...
      const studentDetailPromises = idArray.map(id => student_auth.findById(id));
      ...
      res.render('adminBookRequestList.ejs', { contentCheck, data: mappedData });
    } else {
  ...
  module.exports.admin_book_transaction_log_get = async (req,res) => {
    try{
      ...
      const combinedData = logData.map(log => {
        const student = studentData.find(student => student._id.toString() === log.student_auth_id.toString());
        const book = bookData.find(book => book._id.toString() === log.book_details_id.toString());
...
module.exports.admin_profile_get = async (req,res)=>{
  ...
  try{
   ...
   const fictionDataCount = await book_details.countDocuments({genre: "fiction"});
   const biographyDataCount = await book_details.countDocuments({genre: "biography"});
   const historyDataCount = await book_details.countDocuments({genre: "history"});
   const scienceDataCount = await book_details.countDocuments({genre: "science"});
   const comicDataCount = await book_details.countDocuments({genre: "comic"});
   
   const allGenreCount = {fictionDataCount:fictionDataCount, biographyDataCount: biographyDataCount, historyDataCount : historyDataCount, scienceDataCount: scienceDataCount, comicDataCount: comicDataCount};
...
return {signedURL, name : info.name, author: info.author, id: info._id, genre: info.genre};
