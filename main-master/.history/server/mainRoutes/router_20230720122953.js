...
const { requireStudentAuth, requireAdminAuth } = require('../authentication/authMiddleware.js');
...
route.get('/new-book-registration-form',requireAdminAuth,renderPages.new_book_register_form_get);

// Add Similar Book Form
route.get('/similar-book-add-form/:id',requireAdminAuth,renderPages.similar_book_add_form_get);

// Specific book buy page
route.get('/specific-book-student/:id',requireStudentAuth,renderPages.specific_book_buy_page_get);

// Book Gallery
route.get('/book-gallery',renderPages.book_gallery_get);

// Student Profile
route.get('/student-profile/:id',requireStudentAuth, renderPages.student_profile_get);

// Student Notifications
route.get('/student-notifications',requireStudentAuth, renderPages.student_notifications_get);

// Book Request List
route.get('/book-request-list',requireAdminAuth, renderPages.book_request_list_get);

// Specific book manualId list for request
route.get('/show-list-manualId-specific-book-request/:studentId/:bookId', requireAdminAuth,renderPages.book_request_manualId_list);

// Admin book details list
route.get('/all-books-info' ,requireAdminAuth, renderPages.admin_all_books_list_get);

// Admin manualId book details list
route.get('/specific-book-manaulId-list/:id',requireAdminAuth, renderPages.admin_books_manaulId_list_get);

// Admin book update page
route.get('/admin-book-update-page/:id',requireAdminAuth, renderPages.admin_book_update_page_get);

// Admin transaction log
route.get('/book-transaction-logs',requireAdminAuth, renderPages.admin_book_transaction_log_get);

// Student List data
route.get('/student-list-admin',requireAdminAuth, renderPages.student_details_list_get);

// Student update page
route.get("/specific-student-admin-view/:id",requireAdminAuth, renderPages.student_admin_view_page_get);

// Admin student account request page
route.get('/account-request-list',requireAdminAuth, renderPages.admin_student_account_request_list_get);

// Genre wise book render page
route.get('/books/genre/:id', renderPages.book_top_genre_page_get);

// Admin Profile Page
route.get('/admin-profile/:id',requireAdminAuth, renderPages.admin_profile_get);

//// API
// New Book Registration 
route.post('/api/new-book-registration', upload.single('file'), async (req,res,next) =>{    
  try{
    if(!req.body){
...
route.post('/api/add-existing-book-form/:id', async (req,res)=>{
    ...
    let available_Count;
       ...
       const response = await axios.get(`${ip}${port}/api/all_books_details?id=${id}`);
      available_Count = response.data.availableCount;
      available_Count = available_Count + 1;
      book_Count = response.data.quantity;
      book_Count = book_Count + 1;
      const updateData = await book_details.findByIdAndUpdate(response.data._id, {quantity: book_Count, availableCount: available_Count}, {new: true});
...
res.send(data);
    ...
    if(!data){
        res.status(400).send({message: "Not found information with id "+id});
    }
    else{
        res.send(data);
    }
    ...
    const deleteZeroQuantityBooks = await book_details.deleteMany({quantity: 0});
...
try{
    ...
    if(studentResponse.book_limit_counter < 5){
     ...
     const bookRequestCheckResponse = await request_details_books.countDocuments({student_auth_id:studentResponse._id });
        ...
        let failure = { message : 'Book Request Limit Exceeded: Last 3 request pending!!!' };
     
         res.status(400).json({ failure });
    }
    }
    else{
        let failure = { message : 'Book Storage Limit Exceeded: Cannot keep more than 5 books!!!' };
...
try{
    ...
    const responseSearch = await book_details.findOne({name: name, author: author});
...
const notificationResponse = await student_notifications.deleteMany({student_auth_id: userID});
res.redirect("/student-notifications");
}
else{
    
    console.log("ye he id" + userID);
 const notificationResponse = await student_notifications.find({student_auth_id: userID}).sort({_id:-1});
...
route.get('/api-admin-book-accept/:studentId/:bookId/:manualId', async (req, res) => {
    try {
      const studentId = req.params.studentId;
      const bookId = req.params.bookId;
      const manualId = req.params.manualId;
  
      const bookRequestDelete = await request_details_books.deleteOne({ student_auth_id: studentId, book_details_id: bookId });
    ...
    availableCounter = availableCounter - 1;
      ...
      await book_details.findByIdAndUpdate(bookId, { readCount: readCounter, availableCount : availableCounter}, {new :true});
      ...
      const checkNotificationCount = await student_notifications.find({ student_auth_id: studentId });
      ...
      let limitCounter;
      const studentDetailForLimitCounter = await student_auth.findById(studentId);
      limitCounter = studentDetailForLimitCounter.book_limit_counter;
      limitCounter = limitCounter + 1;
  
      await student_auth.findByIdAndUpdate(studentId, { book_limit_counter: limitCounter }, { new: true });
  
      await all_books.findOneAndUpdate({ admin_id_manual: manualId }, { status: 0 }, { new: true });

  
        if(bookDetailsForNotification.availableCount === 0){

      const notificationData = await request_details_books.find({ book_details_id: bookDetailsForNotification._id });
  
      const student_auth_ids = notificationData.map(notification => notification.student_auth_id);
  
      const existingNotifications = await student_notifications.find({ student_auth_id: { $in: student_auth_ids } });
  
      const matchedStudentAuthIds = existingNotifications.map(notification => notification.student_auth_id);
      const unmatchedStudentAuthIds = student_auth_ids.filter(id => !matchedStudentAuthIds.includes(id));
  
      if (matchedStudentAuthIds.length > 0) {
        const newNotifications = matchedStudentAuthIds.map(student_auth_id => ({
          student_auth_id,
          ...
          message_count: existingNotifications.find(notification => notification.student_auth_id === student_auth_id).message_count + 1,
        }));
  
        await student_notifications.insertMany(newNotifications);
        await request_details_books.deleteMany({ student_auth_id: { $in: matchedStudentAuthIds } });
      }
  
      if (unmatchedStudentAuthIds.length > 0) {
        const newNotifications = unmatchedStudentAuthIds.map(student_auth_id => ({
          student_auth_id,
          book_request_status: `Your Book ${bookDetailsForNotification.name} by author ${bookDetailsForNotification.author} is rejected by admin.`,
          message_count: 1,
        }));
        ...
        await student_notifications.insertMany(newNotifications);
        await request_details_books.deleteMany({ student_auth_id: { $in: unmatchedStudentAuthIds } });
      }
    }
...
await addNewNotification.save();
    ...
    await addNewNotification.save();
       ...
       await request_details_books.deleteOne({student_auth_id: studentId, book_details_id: bookId});
       res.redirect('/book-request-list');
...
route.post('/admin-search-form-student-list', async (req,res)=>{
    try{
        ...
        const responseSearch = await student_auth.findOne({name: name, roll_no: rollNo, year:year, branch: branch });
...
try{
    ...
    await book_details.findByIdAndUpdate(bookId , {quantity : quantityCount}, {new: true});
    ...
    if(manualIdResponse.status === 1){
         ...
         availableCounter = availableCounter - 1;
         await book_details.findByIdAndUpdate(bookId, {availableCount: availableCounter});
    }
    else if(manualIdResponse.status === 0){

const findUser = await student_book_storage.find({admin_id_manual_list : manualIdResponse.admin_id_manual});
...
await student_auth.findByIdAndUpdate(user_Id,{book_limit_counter : bookLimitCounter},{new :true});
await student_book_storage.deleteOne({student_auth_id: user_Id, admin_id_manual_list: manualIdResponse.admin_id_manual});
    }
...
try{
    ...
    const id = req.params.id;
    const storageCheck = await student_book_storage.find({student_auth_id: id});
    if(storageCheck.length){
        let failure = { message : 'This student Acc. contains some book, delete all books from this Acc. first!', id: id };
...
// Book return api
route.get('/api-admin-book-return/:bookId/:adminManualId/:studentId', async (req,res)=>{
try{
const bookId = req.params.bookId;
...
await student_book_storage.deleteOne({student_auth_id: studentId, admin_id_manual_list: adminManualId});

// status ko active kardo manualId book ka
await all_books.updateOne({ admin_id_manual : adminManualId}, {status: 1});
...
availableCounter = availableCounter + 1;
await book_details.findByIdAndUpdate(bookId, {availableCount: availableCounter}, {new :true});

// Logs k database se delete karo
await logs_book_issue.deleteOne({student_auth_id: studentId, admin_id_manual: adminManualId, book_details_id: bookId});
...
// Book Update API
route.post('/api/admin-book-update/:id',  upload.single('file'), async (req,res)=>{
try{
...
await book_details.findByIdAndUpdate(id, {name: req.body.name, author: req.body.author, description: req.body.description, genre: req.body.option});
...
await book_details.findByIdAndUpdate(id, {name: req.body.name, author: req.body.author, description: req.body.description, genre: req.body.option, image_bucket_name: gcBucketFileName});
...
// Student Update api
route.post('/api/admin-student-update/:id', upload.single('file'), async (req,res)=>{
    try{
        ...
        await student_auth.findByIdAndUpdate(id, {name: req.body.name, roll_no: req.body.rollNo, year: req.body.year, branch: req.body.branch, email: req.body.email});
        ...
        await student_auth.findByIdAndUpdate(id, {name: req.body.name, roll_no: req.body.rollNo, year: req.body.year, branch: req.body.branch, email: req.body.email, image_bucket_name: gcBucketFileName});
...
route.post("/api/admin-profile-update/:id", upload.single('file'), async (req,res)=>{
    ...
    try{
        ...
        await admin_auth.findByIdAndUpdate(id, {name: req.body.name, email: req.body.email});
        ...
        await admin_auth.findByIdAndUpdate(id, {name: req.body.name, email: req.body.email, image_bucket_name: gcBucketFileName});
...
route.get("/api/books/genre/:id", async (req, res)=>{
    try{
        ...
        const details = await book_details.find({genre: "fiction"}).sort({ readCount: -1 });  
        ...
        res.send(details);
          ...
          const details = await book_details.find({genre: "history"}).sort({ readCount: -1 });  
          ...
          const details = await book_details.find({genre: "biography"}).sort({ readCount: -1 });  
          ...
          const details = await book_details.find({genre: "science"}).sort({ readCount: -1 });  
          ...
          const details = await book_details.find({genre: "comic"}).sort({ readCount: -1 });  
          ...
          res.status(500).send({message: err.message || "Some error occured while performing this operation"});
        }
});
...
// Admin Registration
route.post('/admin-signup', upload.single('file'), adminAuthController.admin_signup_post);

// Admin Login
route.post('/admin-login',adminAuthController.admin_login_post);

// Student Registration
route.post('/student-signup', upload.single('file'), studentAuthController.student_signup_post);
